...
| 系统 | 安装包 |
| --- | --- |
| Windows 10/11 x64 | [video-toolkit-windows-x64-v1.7.60.zip](https://github.com/secure-artifacts/video-toolkit/releases/download/v1.7.60/video-toolkit-windows-x64-v1.7.60.zip) · [安装包 Setup](https://github.com/secure-artifacts/video-toolkit/releases/download/v1.7.60/VideoToolkit_Setup_v1.7.60.exe) |
| Linux x64 | [video-toolkit-linux-x64-v1.7.60.zip](https://github.com/secure-artifacts/video-toolkit/releases/download/v1.7.60/video-toolkit-linux-x64-v1.7.60.zip) |
| macOS Apple Silicon | [video-toolkit-macos-arm64-v1.7.60.zip](https://github.com/secure-artifacts/video-toolkit/releases/download/v1.7.60/video-toolkit-macos-arm64-v1.7.60.zip) |
| macOS Intel | [video-toolkit-macos-x64-v1.7.60.zip](https://github.com/secure-artifacts/video-toolkit/releases/download/v1.7.60/video-toolkit-macos-x64-v1.7.60.zip) |
...
### v1.7.60 · 2026-09-03
- **安装版跟读**：打包强制打进跟读模块；缺词轴会提示；提取自动写 `*.words.srt`（所有工程通用，不必每个工程单独导）。
- 含 v1.7.59：换预设/改描边跟读不乱。

### v1.7.59 · 2026-08-30
- **换预设/改描边跟读不乱**：跟读始终锚定词级 ASR；词数不一致时按开口映射，不再句长均分。
- **已有词轴时换预设**：只换颜色/字体/效果，保留每句词数与每行字符，避免重切句导致跟读飞掉。
- 含 v1.7.58：Qt 烧录、裁剪后重提字幕、经典黄只变色不放大。

### v1.7.58 · 2026-08-30
- **预览≈导出（Qt 烧录）**：字幕用与预览同一套 Qt 引擎烧录，词间距/样式更一致；ASS 仅作回退。
- **裁剪后对口型**：切片后可自动/手动按成品音轨重提字幕；保留源轴与即时重映射预览。
- **跟读对口型**：恢复严格跟词级 ASR（默认不抢拍、不百分比拉伸）；经典黄只变色不放大；描边 `#222222`。
- 含 v1.7.57：FB Reel 黄/红预设、词间距修复、元数据旋转校正。

### v1.7.57 · 2026-08-24
- **FB Reel 字幕预设**：黄字跟读；红字跟读（语义大小号 + 逐词弹出 + 当前词亮红放大）。
- **预览≈导出词间距**：排版统一用解析后的字体族；去掉虚高字宽；ASS 双写 `\fsp`。
- **清除元数据 · 旋转校正**：支持 180°（倒着拍放正）以及 90° 顺/逆时针，真正改像素。

### v1.7.56 · 2026-08-22
- **分组合成跨电脑排序**：禁止「文件名自然序 + 智能/文案边界」按序号硬套文案；裁剪前按识别结果对齐分段文案。
- 文件名顺序与文案最佳顺序不一致时日志明确提醒改用「按分段文案自动匹配」。
- 分组文案按文件夹名双写，换盘符/换电脑后更容易找回对应表；排序/裁剪提示区分更清晰。
...
### v1.7.54 · 2026-08-21
- **图文成片音轨**：默认无背景音；外部配音路径可靠保存；成品预览/再导出不再叠全局 BGM（修复铃声怪声、双人声）。
- 随机 BGM 只选纯音频；配音强制单声道同相；过短「铃声」外部音频会明确报错。

### v1.7.53 · 2026-08-21
- **检查更新下载**：官方失败自动尝试镜像；校验文件完整性；失败可一键打开 Releases 手动下载。
- Release 说明补齐 Windows / macOS / Linux 下载链接。

### v1.7.52 · 2026-08-21
- **批量字幕提取**：修复切换视频时把上一片字幕写进新片缓存，导致每片都显示同一段字。
- **BGM / 声音合成记忆**：完整记住合成方式、音量、淡入淡出、BGM 路径与选取方式；重启自动恢复。
- **批量字幕预设颜色**：改预设/颜色/字号时清除残留独立样式；导出以批量快照打底。
- **分组合成希腊语**：同形折叠、停用词、近并列拒绝与智能混合边界更严，匹配失败不按索引硬裁。
- **预览≈导出**：系统字体进 fontsdir；校对保留源时钟；跟读/口型时钟与烧录对齐（字幕渲染 v14）。
- **Reels**：PNG 图层排序/删除、调色、人声降噪增强、片段变速等（含前序修复）。

### v1.7.51 · 2026-08-20
- **清理元数据**：损坏容器（STSC/STCO / Invalid data）自动抢救，优先流复制不伤画质；失败跳过并继续批处理；中文错误提示。

### v1.7.50 · 2026-08-20
- **分组文案排序**：弱匹配禁止强行重排；严格连续序号分段。
- **Reels 重命名核对**：导出区标题列表增加编号对照表（队列 ↔ 第 N 行）。
- **格式转换**：原「图片工具」更名；新增视频压缩/转格式（优先转封装保画质）。
- **批量重命名页**：预览增加「对应标题」列。
...
- **字幕口型**：修复裁剪后源时钟/成片时钟混用导致的口型错位。
- **成片画质**：默认 CPU 编码升为 fast；图文中间段更清晰；元数据重编码提高画质。
- **分组文案排序**：按分段文案改为全局最优匹配；支持 `1.` / `1、` 序号分段。
- **Whisper 幻觉过滤**：自动去掉「Υπότιτλοι AUTHORWAVE」等字幕社水印行。

### v1.7.47 · 2026-08-17
- **TikTok/抖音批量字幕**：下载优先合并音视频；下载后检测音轨；无音轨时明确提示（修复「Groq 长音频分段失败 / does not contain any stream」）。
- **帮助更新日志**：已写入 v1.7.47 说明。

### v1.7.46 · 2026-08-16
- **蒙版/PNG 声明轨**：支持「加入当前视频」与「批量全部视频」（类似字幕当前/批量样式）；默认约 5 秒显示。
- **裁剪口型**：保存 ASR 源时钟；视频轨切片后自动重映射句级+词级；预览实时对齐；导出按源轴映射。
- **帮助更新日志**：已写入 v1.7.46 说明。
